...
def load_loop(experiment_dir):
    """Load loop.json if present."""
    path = experiment_dir / "loop.json"
    if not path.exists():
        return None
    try:
...
def format_loop(loop_data):
    """Format loop metadata for compact display."""
    if not loop_data:
        return "—"
    interval = loop_data.get("interval", "?")
    max_iterations = loop_data.get("max_iterations")
    completed = loop_data.get("completed_iterations")
    if max_iterations is not None and completed is not None:
        return f"{interval} ({completed}/{max_iterations})"
    return interval
...
def print_experiment(experiment_dir, experiment_path):
    ...
    if loop_data:
        workflow_file = loop_data.get("workflow_file", ".github/workflows/autoresearch-loop.yml")
        print(f"  Active loop: {format_loop(loop_data)} (workflow: {workflow_file})")
        if loop_data.get("max_iterations") is not None:
            print(
                "  No improvement streak: "
                f"{loop_data.get('consecutive_no_improve', 0)}/{loop_data.get('stop_after_no_improve', 5)}"
            )
...
def print_dashboard(root):
    ...
    for domain_dir in sorted(root.iterdir()):
        ...
        for exp_dir in sorted(domain_dir.iterdir()):
            ...
            experiments.append({
                ...
                "loop": format_loop(load_loop(exp_dir)),
            })
    ...
    print(f"{'─' * 106}")
    print(f"  {'DOMAIN':<15} {'EXPERIMENT':<20} {'RUNS':>5} {'KEPT':>5} {'BEST':>12} {'CHANGE':>10} {'STATUS':<8} {'LOOP':<14}")
    print(f"  {'─' * 101}")
    for e in experiments:
        print(
            f"  {e['domain']:<15} {e['name']:<20} {e['runs']:>5} {e['kept']:>5} "
            f"{e['best']:>12} {e['change']:>10} {e['status']:<8} {e['loop']:<14}"
        )
    print()
    return experiments
    ...
    for domain_dir in sorted(root.iterdir()):
        ...
        for exp_dir in sorted(domain_dir.iterdir()):
            ...
            experiments.append([
                ...
                best_str, pct_str, format_loop(load_loop(exp_dir))
            ])

    buf = io.StringIO()
    writer = csv.writer(buf)
    writer.writerow(["Domain", "Experiment", "Metric", "Runs", "Kept", "Discarded", "Crashed", "Best", "Change", "Loop"])
    for e in experiments:
        writer.writerow(e)
    return buf.getvalue()
...
def export_experiment_markdown(experiment_dir, experiment_path):
    ...
    loop_data = load_loop(experiment_dir)
    if loop_data:
        lines.append(f"**Loop:** `{format_loop(loop_data)}`  ")
        if loop_data.get("max_iterations") is not None:
            lines.append(
                "**No improvement streak:** "
                f"`{loop_data.get('consecutive_no_improve', 0)}/{loop_data.get('stop_after_no_improve', 5)}`\n"
...
def export_dashboard_markdown(root, domain_filter=None):
    ...
    lines.append("| Domain | Experiment | Metric | Runs | Kept | Best | Change | Status | Loop |")
    lines.append("|--------|-----------|--------|------|------|------|--------|--------|------|")
    ...
    for domain_dir in sorted(root.iterdir()):
        ...
        for exp_dir in sorted(domain_dir.iterdir()):
            ...
            loop = format_loop(load_loop(exp_dir))
            lines.append(
                f"| {domain_dir.name} | {exp_dir.name} | {config.get('metric', '?')} | "
                f"{stats['total']} | {stats['keeps']} | {best} | {pct} | {status} | {loop} |"
            )
